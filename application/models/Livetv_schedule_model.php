...
class Livetv_schedule_model extends MY_Model {
    ...
    {
        return $this->db->get_where('livetv_schedule', array($key => $val))->row();
    }
    ...
    public function items($offset = 0, $limit = 50, $s = '', $cid = '')
    {
        ...
        return $this->db->get_where('livetv_schedule', array(), $limit, $offset)->result();
    }
    ...
    public function total($s = '', $cid = '')
    {
        ...
        return $this->db->get_where('livetv_schedule')->num_rows();
    }
        ...
        foreach($post as $key=>$value)
        {
            if(isset($value['code'])){
                ...
                if(!empty($rows)){
                    ...
                    $this->db->insert('livetv_schedule', $data);
    ...
    public function getDateExits($date)
    {
        $this->db->select('id');
        $this->db->where('schedule_date', $date);
        return $this->db->get_where('livetv_schedule', array())->row();
    }
    ...
    {
        $this->db->delete('livetv_schedule', array(
            $key => $val,
        ));
    }
    ...
    {
        $this->db->order_by('id DESC');
        $row = $this->db->get('livetv_schedule')->row();
        $autoid = $row->id + 1;     
        ...
        $this->db->update('livetv_schedule', '', array('id' => $autoid));
    }

    public function getObjectData($_schedule)
    {
        date_default_timezone_set('Asia/Ho_Chi_Minh');
        $obj = array();
        $objItem = array();
        $content = @translate($_schedule->content, $_schedule->content_en);
        $schedule = explode(";",$content);
        $total = count($schedule);
        for($i=0;$i < $total;$i++):
            $item = explode("||",$schedule[$i]);
            $schedule_date = (isset($_schedule->schedule_date)) ? $_schedule->schedule_date : '';
            $time = (isset($item[0])) ? trim($item[0]) : ''; 
            $title = (isset($item[1])) ? trim($item[1]) : '' ;
            $start = strtotime( $schedule_date. ' ' . $time . ':00');
    ...
    public function getScheduleCurrent($schedule)
    {
        //date_default_timezone_set('Asia/Ho_Chi_Minh');
        $obj = array();
        $objItem = array();
        ...
        return $objItem[count($objItem)-1];
    }
    ...
    private function _get_posts($param = array())
    {
        ...
        if(isset($param['livetv_id'])){
            $this->slave->where('livetv_id', $param['livetv_id']);
        }
        return $this->slave->get('livetv_schedule')->row();
    }
