...
class Node_model extends MY_Model {
    ...
    {
        return $this->db->get_where('node', array($key => $val))->row();
    }
    ...
    public function items($extension = '', $offset = 0, $limit = 50, $s = '', $cid = '', $date_from = '', $date_to = '')
    {
        ...
        return $this->db->get_where('node', array('extension' => $extension), $limit, $offset)->result();
    }
    ...
    public function total($extension = '', $s = '', $cid = '', $date_from = '', $date_to = '')
    {
        ...
        return $this->db->get_where('node', array('extension' => $extension))->num_rows();
    }
    ...
    public function save($extension = '', $post, $id = 0)
    {
        ...
        if(empty($post['alias'])  ||  $exist){
            ...
            $this->db->update('node', array('alias' => $alias .'-'. $id), array('id' => $id));
        }
        return $id;
    }
    ...
    {
        $this->db->order_by('id DESC');
        return $this->db->get_where('node', array('extension' => $extension, 'parent' => $id))->result();
    }

    public function get_note_category($extension = '', $alias = '')
    {
        return $this->db->get_where('node', array('extension' => $extension, 'alias' => $alias))->row();
    }

    public function copy($extension = '', $id = 0)
    {
        $this->db->order_by('id DESC');
        $row = $this->db->get('node')->row();
        $autoid = $row->id + 1;
    ...
    public function status($extension = '', $id = 0, $status = '')
    {
        ...
        $this->db->update('node', $data, array('extension' => $extension, 'id' => $id));
    }
