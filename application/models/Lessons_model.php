...
class Lessons_model extends MY_Model
{
    ...
    {
        return $this->db->get_where('lessons', array('alias' => $alias, 'id !=' => $id))->num_rows();
    }

    public function item($val = '', $key = 'id')
    {
        return $this->db->get_where('lessons', array($key => $val))->row();
    }
    ...
    public function items($extension = '', $offset = 0, $limit = 50, $s = '', $cid = '', $date_from = '', $date_to = '')
    {
        ...
        return $this->db->get_where('lessons', array('extension' => $extension), $limit, $offset)->result();
    }
    ...
    public function total($extension = '', $s = '', $cid = '', $date_from = '', $date_to = '')
    {
        ...
        return $this->db->get_where('lessons', array('extension' => $extension))->num_rows();
    }
    ...
    public function save($extension = '', $post, $id = 0)
    {
        ...
        $data = array(
            ...
            'files_video'    => isset($post['files_video']) ? $post['files_video'] : '',
            'id_level'       => isset($post['id_level']) ? $post['id_level'] : '',
            'id_author'      => isset($post['id_author']) ? $post['id_author'] : '',
        ...
        if ($id) {
            ...
            $this->db->update('lessons', $data, array('id' => $id));
        } else {
        ...
        if ($extension == 'category-lessons') {
            ...
            if (empty($post['alias']) || $exist) {
                ...
                $this->db->update('lessons', array('alias' => $alias . '-' . $id), array('id' => $id));
            }
        }
        return $id;
    }
    ...
    public function parent($extension = '', $id = 0, $alt = '')
    {
        ...
        $query = $this->db->get_where('lessons', array('extension' => $extension, 'parent' => $id));
        $rows = $query->result();
        if ($rows) {
            foreach ($rows as $row) {
    ...
    public function parents($extension = '')
    {
        $this->db->order_by('id DESC');
        $this->db->where('status', 1);
        return $this->db->get_where('lessons', array('extension' => $extension))->result();
    }

    public function note_category($extension = '', $id = 0)
    {
        $this->db->order_by('id DESC');
        return $this->db->get_where('lessons', array('extension' => $extension, 'parent' => $id))->result();
    }

    public function get_note_category($extension = '', $alias = '')
    {
        return $this->db->get_where('lessons', array('extension' => $extension, 'alias' => $alias))->row();
    }

    public function copy($extension = '', $id = 0)
    {
        $this->db->order_by('id DESC');
        $row = $this->db->get('lessons')->row();
        $autoid = $row->id + 1;
        $this->db->query("CREATE TEMPORARY TABLE tmp SELECT * FROM #__node WHERE id = " . $id . ";");
        $this->db->query("UPDATE tmp SET id = " . $autoid . " WHERE id = " . $id . ";");
        $this->db->query("INSERT INTO #__node SELECT tmp.* FROM tmp WHERE id = " . $autoid . ";");
        $row = $this->item($autoid, 'id');
        $params['alias'] = $row->alias . '-' . $autoid;
        $this->db->update('lessons', $params, array('id' => $autoid));
    }
    ...
    {
        $this->slave->where(' (date_from = "0000-00-00"  OR  date_from <= "' . date('Y-m-d') . '")  AND  (date_to = "0000-00-00"  OR  date_to >= "' . date('Y-m-d') . '")');
        ...
        $query = $this->slave->get('lessons');
        if ($param['query'] == 'result') {
            $data = $query->result();
    ...
    public function status($extension = '', $id = 0, $status = '')
    {
        ...
        $this->db->update('lessons', $data, array('extension' => $extension, 'id' => $id));
    }
