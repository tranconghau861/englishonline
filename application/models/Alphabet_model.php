...
class Alphabet_model extends MY_Model
{
    ...
    {
        return $this->db->get_where('alphabet', array('alias' => $alias, 'id !=' => $id))->num_rows();
    }

    public function item($val = '', $key = 'id')
    {
        return $this->db->get_where('alphabet', array($key => $val))->row();
    }
    ...
    public function items($extension = '', $offset = 0, $limit = 50, $s = '', $cid = '', $date_from = '', $date_to = '')
    {
        ...
        $result = $this->db->get_where('alphabet', array('extension' => $extension), $limit, $offset)->result();
...
//        print_r($sql); exit;
    ...
    public function total($extension = '', $s = '', $cid = '', $date_from = '', $date_to = '')
    {
        ...
        return $this->db->get_where('alphabet', array('extension' => $extension))->num_rows();
    }
    ...
    public function save($extension = '', $post, $id = 0)
    {
        ...
        if ($id) {
            ...
            $this->db->update('alphabet', $data, array('id' => $id));
        } else {
            $data['created'] = date('Y-m-d H:i:s');
            $data['uid'] = isset($post['uid']) ? $post['uid'] : $admin['id'];
            $this->db->insert('alphabet', $data);
        ...
        if (empty($post['alias']) || $exist) {
            ...
            $this->db->update('alphabet', array('alias' => $alias . '-' . $id), array('id' => $id));
        }
        return $id;
    }
    ...
    public function parent($extension = '', $ids, $alt = '')
    {
        $data = array();
        if ($alt == 0) {
            $this->db->select('id');
        ...
        if (is_array($ids)) {
            $this->db->where_in('parent', $ids);
        } else {
            $this->db->where('parent', $ids);
        }
        $query = $this->db->get_where('alphabet', array('extension' => $extension));
        if ($alt == 0) {
    ...
    public function parents($extension = '')
    {
        ...
        return $this->db->get_where('alphabet', array('extension' => $extension))->result();
    }

    public function note_category($extension = '', $id = 0)
    {
        $this->db->order_by('id DESC');
        return $this->db->get_where('alphabet', array('extension' => $extension, 'parent' => $id))->result();
    }

    public function get_note_category($extension = '', $alias = '')
    {
        return $this->db->get_where('alphabet', array('extension' => $extension, 'alias' => $alias))->row();
    }

    public function copy($extension = '', $id = 0)
    {
        $this->db->order_by('id DESC');
        $row = $this->db->get('alphabet')->row();
        $autoid = $row->id + 1;
        ...
        $this->db->update('alphabet', $params, array('id' => $autoid));
    }
    ...
    public function status($extension = '', $id = 0, $status = '')
    {
        ...
        $this->db->update('alphabet', $data, array('extension' => $extension, 'id' => $id));
    }
