...
class Noun_model extends MY_Model
{
    ...
    {
        return $this->db->get_where('noun', array('alias' => $alias, 'id !=' => $id))->num_rows();
    }

    public function item($val = '', $key = 'id')
    {
        return $this->db->get_where('noun', array($key => $val))->row();
    }
    ...
    public function items($extension = '', $offset = 0, $limit = 50, $s = '', $cid = '', $date_from = '', $date_to = '')
    {
        ...
        return $this->db->get_where('noun', array('extension' => $extension), $limit, $offset)->result();
    }
    ...
    public function total($extension = '', $s = '', $cid = '', $date_from = '', $date_to = '')
    {
        ...
        return $this->db->get_where('noun', array('extension' => $extension))->num_rows();
    }
    ...
    public function save($extension = '', $post, $id = 0)
    {
        ...
        if ($id) {
            ...
            $this->db->update('noun', $data, array('id' => $id));
        } else {
    ...
    public function parent($extension = '', $id = 0, $alt = '')
    {
        ...
        $query = $this->db->get_where('noun', array('extension' => $extension, 'parent' => $id));
        $rows = $query->result();
        if ($rows) {
            foreach ($rows as $row) {
    ...
    public function parents($extension = '')
    {
        ...
        return $this->db->get_where('noun', array('extension' => $extension))->result();
    }

    public function note_category($extension = '', $id = 0)
    {
        $this->db->order_by('id DESC');
        return $this->db->get_where('noun', array('extension' => $extension, 'parent' => $id))->result();
    }

    public function get_note_category($extension = '', $alias = '')
    {
        return $this->db->get_where('noun', array('extension' => $extension, 'alias' => $alias))->row();
    }

    public function copy($extension = '', $id = 0)
    {
        $this->db->order_by('id DESC');
        $row = $this->db->get('noun')->row();
        $autoid = $row->id + 1;
        ...
        $this->db->update('noun', $params, array('id' => $autoid));
    }
    ...
    private function _get_posts($param = array())
    {
        ...
        if (isset($param['id_type'])) {
            $this->slave->where('id_type', $param['id_type']);
        }
        ...
        if (isset($param['random'])) {
            $this->slave->order_by($param['random'], 'RANDOM');
        }
        if (isset($param['like'])) {
            if (isset($param['id_type']) && $param['id_type'] == 2) {
                $this->slave->like('title', $this->db->escape_like_str($param['like']), 'before');
            } else {
                $this->slave->like('title', $param['like']);
            }
        }
        $query = $this->slave->get('noun');
    ...
    public function status($extension = '', $id = 0, $status = '')
    {
        ...
        $this->db->update('noun', $data, array('extension' => $extension, 'id' => $id));
    }
