...
class Category_model extends MY_Model {
    ...
    {
        return $this->db->get_where('category', array('alias' => $alias, 'id !=' => $id))->num_rows();
    }
...
//            $data['created'] = date('Y-m-d H:i:s');
        ...
        if(empty($post['alias'])  ||  $exist){
            ...
            $this->db->update('category', array('alias' => $alias .'-'. $id), array('id' => $id));
        }
        return $id;
    }
...
//    }
    ...
    public function copy($extension = '', $id = 0)
    {
        ...
        $autoid = $row->id + 1;
        $this->db->query("CREATE TEMPORARY TABLE tmp SELECT * FROM #__category WHERE id = ". $id .";");
        $this->db->query("UPDATE tmp SET id = ". $autoid ." WHERE id = ". $id .";");
        $this->db->query("INSERT INTO #__category SELECT tmp.* FROM tmp WHERE id = ". $autoid .";");
