...
class Livevideo_schedule extends CI_Controller {
	...
	public function index($extension = '', $offset = 0)
	{
		...
		$view['rows'] = $this->livevideo_schedule->items($extension, $offset, $limit, $s, $cid);
		...
		$this->pagination->initialize(array(
			'base_url' 		=> site_url('admin/livevideo_schedule/index/'. $extension),
			'total_rows' 	=> $total,
			'per_page' 		=> $limit,
		));
		...
		$this->load->view('admin/livevideo_schedule/livevideo_schedule_list', $view);
	}
	...
	public function form($extension = '', $id = 0)
	{
			...
			if(!empty($post['hd_player_lst']) && !empty($post['schedule_date']))
			 {
				...
				if($id==0){
					$lrow = $this->livevideo_schedule->getDateExits($post['schedule_date']);
		...
		$this->load->view('admin/header');
		$this->load->view('admin/livevideo_schedule/livevideo_schedule_form', $view);
	}
	...
	public function language($extension = '', $id = 0)
	{
		...
		$this->load->view('admin/header');
		$this->load->view('admin/livevideo_schedule/livevideo_schedule_lang', $view);
	}
