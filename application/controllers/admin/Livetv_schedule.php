...
class Livetv_schedule extends CI_Controller {
	...
	public function index($extension = '', $offset = 0)
	{
		...
		if($post = $this->input->post())
		{
			foreach($post['id'] as $id)
			{
				...
				$this->livetv_schedule->delete($id);
			}
		...
		$view['rows'] = $this->livetv_schedule->items($offset, $limit, $s, $cid);
		$view['total'] = $total = $this->livetv_schedule->total($s, $cid);
		...
		$this->pagination->initialize(array(
			'base_url' 		=> site_url('admin/livetv_schedule/index/'. $extension),
			'total_rows' 	=> $total,
			'per_page' 		=> $limit,
		));
		...
		$this->load->view('admin/livetv_schedule/livetv_schedule_list', $view);
	}
	public function import($extension = '', $id = 0)
	{
		...
		$row = $this->livetv_schedule->item($id);
		if($post = $this->input->post())
		{				
	...
	public function form($extension = '', $id = 0)
	{
		...
		if($post = $this->input->post())
		{				
			if(!empty($post['livetv_id']) && !empty($post['hd_player_lst']) && !empty($post['schedule_date']))
			 {
				$flat = 0;
			
				if($id==0){
					$lrow = $this->livetv_schedule->getDateExits($post['schedule_date']);
				...
				if($flat == 1){
					...
					$this->livetv_schedule->save($extension, $post, $id);
			...
			if(empty($post['livetv_id']) && empty($post['hd_player_lst']) && empty($post['schedule_date'])) 
			{
				...
				if(empty($post['livetv_id'])){
					
					$view['msg'] = 'Vui lòng chọn kênh.';
		...
		$view['category'] = $this->livetv->items('livetv');
		$this->load->view('admin/header');
		$this->load->view('admin/livetv_schedule/livetv_schedule_form', $view);
	}
	...
	{		
		...
		$view['schedule'] = $this->livetv_schedule->get_posts(
			array(
				'livetv_id' => 7,
		...
		echo $this->load->view('admin/livetv_schedule/view_schedule', $view, TRUE);
	}
	...
	public function language($extension = '', $id = 0)
	{
		...
		if($post = $this->input->post())
		{
			$this->livetv_schedule->language($extension, $post, $id);
			redirect('admin/livetv_schedule/index/'. $extension);
		}
		...
		$this->load->view('admin/header');
		$this->load->view('admin/livetv_schedule/livetv_schedule_lang', $view);
	}
	...
	public function copy($extension = '', $id = 0)
	{
		...
		if($id){
			$this->livetv_schedule->copy($extension, $id);
		}
		redirect('admin/livetv_schedule/index/'. $extension);
	}
