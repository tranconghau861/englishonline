...
class Alphabet extends CI_Controller {
    ...
    public function index($extension = '', $offset = 0)
    {
        ...
        if($post  &&  isset($admin['params'][$extension]['delete']))
        {
            foreach($post['id'] as $id)
            {
                ...
                $this->alphabet->delete($id);
            }
            redirect('admin/alphabet/index/'. $extension);
        }
        ...
        $view['rows'] = $this->alphabet->items($extension, $offset, $limit, $s, $cid, $date_from, $date_to);
        $view['total'] = $total = $this->alphabet->total($extension, $s, $cid, $date_from, $date_to);
        ...
        $view['category'] = $this->alphabet->parent('category-'. $extension);
        
        $this->pagination->initialize(array(
            'base_url'      => site_url('admin/alphabet/index/'. $extension),
            'total_rows'    => $total,
            'per_page'      => $limit,
        ));
        ...
        $this->load->view('admin/alphabet/alphabet_list', $view);
    }
    ...
    public function form($extension = '', $id = 0)
    {
        ...
        if($post = $this->input->post())
        {
            if(!$post['title']) {
				...
				$this->mp_cache->delete_group('cache.alphabet_');
				
                $this->alphabet->save($extension, $post, $id);
                redirect('admin/alphabet/index/'. $extension);
            }
        }
        ...
        $this->load->view('admin/header');
        $this->load->view('admin/alphabet/alphabet_form', $view);
    }
    ...
    public function language($extension = '', $id = 0)
    {
        ...
        if($post = $this->input->post())
        {
            if(!$post['title_en']) {
                ...
                $this->alphabet->language($extension, $post, $id);
                redirect('admin/alphabet/index/'. $extension);
            }
        }
        ...
        $this->load->view('admin/header');
        $this->load->view('admin/alphabet/alphabet_lang', $view);
    }
    ...
    public function copy($extension = '', $id = 0)
    {
        ...
        if($id){
            $this->alphabet->copy($extension, $id);
        }
        redirect('admin/alphabet/index/'. $extension);
    }
    ...
    {   
        ...
        $this->alphabet->status($extension, $id, $status);
        redirect('admin/alphabet/index/'. $extension);
    }
    ...
    public function filter()
    {
        if($post = $this->input->post())
        {
            ...
            $rows = $this->alphabet->get_posts(array(
                'query'         => 'result',
                'extension'     => 'news',
                'status'        => 1,
