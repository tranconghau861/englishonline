            ...
            <div class="row">
                ...
                <div class="col-md-4 button">
                    ...
                    <a href="<?=base_url('admin/alphabet/form/'. $extension)?>" class="btn btn-success"><i class="fa fa-plus"></i> Thêm</a>
                    <?php } ?>
                    ...
                    <table class="table table-bordered">
                        ...
                        <?php
                            if($rows){
                                $i = 1;
                                foreach($rows as $row){
                                    $parent = $this->alphabet->item($row->parent);
                        ?>
                            <tr>
                                ...
                                <td>
                                    <?=$row->title?><br/>
                                    <?php if(in_array($extension, array('category-alphabet'))  &&  $parent){ ?>
                                        <span class="text-muted">Main:</span> <span class="text-danger"><?=$parent->title?></span>
                                    <?php } ?>
		                        ...
		                        <td class="text-center">
			                        <?php echo @$row->photo ? '<a href="'. get_image_link($row->photo, 100, 70) .'" class="manific-image"><img src="'. get_image_link($row->photo) .'" width="100" height="70"></a>' : ''; ?>
		                        </td>
								<?php } ?>
								
                                <?php if($islang && in_array($extension, $modlang)){ ?>
                                <th class="text-center"><a href="<?=base_url('admin/alphabet/language/'. $extension .'/'. $row->id)?>" data-toggle="tooltip" data-placement="top" title="Làm một bản dịch sang tiếng Anh"><img src="<?=base_url('assets/admin/images/en.png')?>"></a></th>
                                <?php } ?>
                                ...
                                <td class="text-center">
                                    <p>
                                        ...
                                        <a href="<?=base_url('admin/alphabet/form/'. $extension .'/'. $row->id)?>" data-toggle="tooltip" data-placement="top" title="Sửa"><i class="fa fa-pencil"></i></a> &nbsp; 
                                        <?php } ?>
                                        
                                        <?php if(isset($admin['params'][$extension]['delete'])){ ?>
                                        <a href="<?=base_url('admin/alphabet/delete/'. $extension .'/'. $row->id)?>" data-toggle="tooltip" data-placement="top" title="Xoá" class="delrow"><i class="fa fa-trash-o"></i></a> &nbsp; 
                                        <?php } ?>
                                        
                                        <?php if(isset($admin['params'][$extension]['add']) || isset($admin['params'][$extension]['edit'])){ ?>
                                        <a href="<?=base_url('admin/alphabet/copy/'. $extension .'/'. $row->id)?>" data-toggle="tooltip" data-placement="top" title="Copy"><i class="fa fa-copy"></i></a>
                                        <?php } ?>
                                    </p>
                                    
                                    <p>
                                        <?php if(isset($admin['params'][$extension]['edit'])){ ?>
                                        <a href="<?=base_url('admin/alphabet/status/'. $extension .'/'. $row->id .'/1')?>" data-toggle="tooltip" data-placement="top" title="Xuất bản"><i class="fa fa-refresh"></i></a> &nbsp; 
                                        <?php } ?>
                                        
                                        <?php if(isset($admin['params'][$extension]['edit'])){ ?>
                                        <a href="<?=base_url('admin/alphabet/status/'. $extension .'/'. $row->id .'/0')?>" data-toggle="tooltip" data-placement="top" title="Chờ duyệt"><i class="fa fa-arrow-circle-o-down"></i></a> &nbsp; 
                                        <?php } ?>
