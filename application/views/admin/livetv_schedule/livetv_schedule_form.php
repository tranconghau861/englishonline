			...
			<form method="post" name="adminForm" class="form-horizontal">
				...
				<div class="form-group">
					...
					<div class="col-sm-3">
	                    <select name="livetv_id" class="form-control">
	                        <option value=""></option>
	                        <?php if($category){ foreach($category as $cat){ ?>
	                        <option value="<?=$cat->id?>" <?=$cat->id==@$row->livetv_id?'selected':''?>><?=$cat->name?></option>
	                        <?php }} ?>
	                    </select>
					</div>
				</div>			
				...
				<div class="form-group">
					...
					<div class="col-sm-6">
						...
						<input type="text" class="form-control datetimepicker" name="schedule_date" value="<?=(!empty($row->schedule_date))?date('d-m-Y',strtotime(trim($row->schedule_date))):''?>" placeholder="Ngày phát sóng">
						<span class="input-group-addon">
							<span class="glyphicon glyphicon-calendar"></span>
						</span>
					</div>
