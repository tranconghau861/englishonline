			...
			<div class="row">
				<div class="col-md-8">
					<form class="form-inline" method="get">
						...
						<?php if(in_array($extension, array('news', 'video', 'relax', 'notice', 'career'))){ ?>
						<div class="input-group" style='margin-top:10px;'>
							<div class="input-group-btn">
								<div class="input-group date" id="date_from" style='width:50%;'>
									<input type="text" class="form-control" name="date_from" value="<?=(int)@$date_from?$date_from:''?>" placeholder="Bắt đầu">
									<span class="input-group-addon">
										<span class="glyphicon glyphicon-calendar"></span>
									</span>
								</div>
								<div class="input-group date" id="date_to"  style='width:50%; padding:5px;'>
									<input type="text" class="form-control datetimepicker" name="date_to" value="<?=(int)@$date_to?$date_to:''?>" placeholder="Kết thúc">
									<span class="input-group-addon">
										<span class="glyphicon glyphicon-calendar"></span>
									</span>
								</div>
				...
				<div class="col-md-4 button">
		            ...
		            <a href="<?=base_url('admin/node/form/'. $extension)?>" class="btn btn-success"><i class="fa fa-plus"></i> Thêm</a>
		            <?php } ?>
		            ...
		            <table class="table table-bordered">
		                <thead>
		                    <tr>
								...
								<?php if(in_array($extension, array('category-news', 'category-video', 'category-relax'))){ ?>
								...
								<?php if(in_array($extension, array('news', 'video', 'relax', 'about', 'notice', 'career'))){ ?>
		                ...
		                ?>
		                    <tr>
		                        ...
		                        <td>
									<?php
										if($parent && in_array($extension, array('category-news', 'category-video', 'category-relax'))){
									...
									<?php if(in_array($extension, array('news', 'video', 'relax'))  &&  $parent){ ?>
										<span class="text-muted">Chuyên mục:</span> <span class="text-danger"><?=$parent->title?></span>
									<?php } ?>
		                        ...
		                        <td class="text-center">
									<?php
										if(in_array($extension, array('news', 'video', 'relax', 'notice', 'career'))){
											if($row->status == 1){
		                        ...
		                        <td class="text-center">
			                        <p>
				                        ...
				                        <a href="<?=base_url('admin/node/form/'. $extension .'/'. $row->id)?>" data-toggle="tooltip" data-placement="top" title="Sửa"><i class="fa fa-pencil"></i></a> &nbsp;
				                        <?php } ?>

										<?php if(isset($admin['params'][$extension]['delete'])){ ?>
				                        <a href="<?=base_url('admin/node/delete/'. $extension .'/'. $row->id)?>" data-toggle="tooltip" data-placement="top" title="Xoá" class="delrow"><i class="fa fa-trash-o"></i></a> &nbsp;
				                        <?php } ?>

										<?php if(isset($admin['params'][$extension]['add']) || isset($admin['params'][$extension]['edit'])){ ?>
				                        <a href="<?=base_url('admin/node/copy/'. $extension .'/'. $row->id)?>" data-toggle="tooltip" data-placement="top" title="Copy"><i class="fa fa-copy"></i></a>
				                        <?php } ?>
			                        </p>
									...
									<p>
										<?php if(isset($admin['params'][$extension]['edit'])){ ?>
										<a href="<?=base_url('admin/node/status/'. $extension .'/'. $row->id .'/1')?>" data-toggle="tooltip" data-placement="top" title="Xuất bản"><i class="fa fa-refresh"></i></a> &nbsp;
				                        <?php } ?>

										<?php if(isset($admin['params'][$extension]['edit'])){ ?>
										<a href="<?=base_url('admin/node/status/'. $extension .'/'. $row->id .'/0')?>" data-toggle="tooltip" data-placement="top" title="Chờ duyệt"><i class="fa fa-arrow-circle-o-down"></i></a> &nbsp;
				                        <?php } ?>
